...
# Colors
BG = (247, 243, 234)     # warm cream — halftone reads as paper fiber, not dirt
INK = (38, 50, 79)
READ = (163, 155, 139)
META = (154, 147, 136)
FADE = (248, 244, 232)
...
SCENE_COLOR = (183, 173, 156)
STORY_COLOR = (167, 156, 138)
HAIRLINE_COLOR = (201, 195, 183)
...
def main():
    ...
    # plain top fade (deckle rejected — "I like my style without the torn up thing")
    yy = np.arange(H)[:, None].astype(np.float64)
    fade_top, fade_len = 130.0 * U, 175.0 * U
    mask_arr = np.clip((yy - fade_top) / fade_len, 0, 1) ** 1.5
    mask = Image.fromarray((np.repeat(mask_arr, W, axis=1) * 255).astype(np.uint8), "L")
    ...
    if SAMPLE_END > 0:
        out_file = OUTPUT_DIR / f"lab_cream_{SAMPLE_START:.0f}s.mp4"
