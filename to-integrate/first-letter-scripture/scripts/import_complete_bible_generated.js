#!/usr/bin/env node
/**
 * Import all generated pericopes from scripts/pericopes_generated into DB.
...
// Import from main pericopes directory (newly generated with enhanced titles)
...
import { CHRONICLES1_PERICOPES } from './pericopes/13_1chronicles.js';
import { CHRONICLES2_PERICOPES } from './pericopes/14_2chronicles.js';
...
async function importPericopes(bookName, pericopes) {
  try {
    ...
    // Clean slate
    await supabase.from('bible_pericopes').delete().eq('book_id', bookId);
    ...
    console.error(`❌ ${bookName}: ${e.message}`);
    return false;
  }
}
