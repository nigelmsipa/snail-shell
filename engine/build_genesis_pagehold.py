...
def draw_centered_text(draw, text, font, y_center, color, letter_spacing=0):
    if letter_spacing > 0:
        total_w = sum(draw.textlength(ch, font=font) for ch in text) + letter_spacing * max(0, len(text) - 1)
        cx = (W - total_w) / 2
        for ch in text:
            draw.text((cx, y_center), ch, font=font, fill=color, anchor="lm")
            cx += draw.textlength(ch, font=font) + letter_spacing
    else:
        draw.text((W/2, y_center), text, font=font, fill=color, anchor="mm")
...
def main():
    ...
    out_file = OUTPUT_DIR / "sample_layout.mp4"
    ...
    for frame in range(start_frame, end_frame):
        ...
        for r in rows:
            ...
            if r['type'] == 'scene':
                ...
                draw_centered_text(draw_text, r['text'], SCENE_MARKER_FONT, cursor_y + 52 + 12.5, SCENE_COLOR, letter_spacing=5.5)

            elif r['type'] == 'story':
                line_y = cursor_y + 80
                draw_text.rectangle([W/2 - 40, line_y, W/2 + 40, line_y + 2], fill=HAIRLINE_COLOR)
                draw_centered_text(draw_text, r['text'], STORY_MARKER_FONT, cursor_y + 80 + 2 + 30 + 22, STORY_COLOR)
                ...
                for item in r['items']:
                    itype, itext, iw, ref = item
                    if itype == 'vn':
                        draw_text.text((cursor_x + space_w*0.1, baseline - 50), itext, font=SANS_VN, fill=ACCENT, anchor="ls")
                        cursor_x += iw + space_w
                    else:
                        w_idx = ref
                        if w_idx == active_w:
                            ...
                            draw_text.rounded_rectangle([hx1, hy1, hx2, hy2], radius=4, fill=HL)
        ...
        if (frame - start_frame) % 300 == 0:
            print(f"Rendered {frame - start_frame}/{end_frame - start_frame} frames...")
